...
namespace StudioDrydock.AppStoreConnect.Cli
{
	internal static class Program
	{
		...
		private static AppStoreClient CreateClient(InvocationContext context, out INestedLog log)
		{
			...
			{
				log = new ConsoleLogger(Console.Error, 3);
			}
			else
			{
				log = new ConsoleLogger(Console.Error, 0);
			}
		...
		private static IAppStoreClientTokenMaker CreateTokenMaker(FileInfo configFile)
		{
			...
			string configDirectory = configFile.Directory?.FullName ?? throw new Exception($"Failed to get directory of {configFile}");
		...
		private static async Task GetApps(InvocationContext context)
		{
			...
			await StdLog(log?.SubPath(nameof(GetApps)), async log =>
			{
				...
				Output(context, new AppInfoList([.. apps.OrderBy(a => a.bundleId)]));
			});
		}
		...
		private static async Task GetAppVersions(InvocationContext context)
		{
			...
			var app = await StdLog(log?.SubPath(nameof(GetAppVersions)), async log =>
			{
				...
				await ForEachAsyncLog(versions, log, v => $"{v.platform}/{v.versionString}", async (version, log) =>
				{
					...
					await ForEachAsyncLog(localizations, log, l => $"{l.locale}", async (loc, log) =>
					{
				...
				return new App(appInfo, [.. versions]);
			});
		...
		private static async Task GetAppIaps(InvocationContext context)
		{
			...
			var iapList = await StdLog(log?.SubPath(nameof(GetAppIaps)), async log =>
			{
				...
				await ForEachAsyncLog(iaps, log, i => $"{i.name}", async (iap, log) =>
				{
				...
				return new IapList([.. iaps.OrderBy(a => a.productId)]);
			});
		...
		private static async Task GetAppEvents(InvocationContext context)
		{
			...
			var events = await StdLog(log?.SubPath(nameof(GetAppEvents)), async log =>
			{
				...
				await ForEachAsyncLog(events, log, e => $"{e.referenceName}", async (ev, log) =>
				{
				...
				return new EventList([.. events.OrderBy(a => a.id)]);
			});
		...
		private static async Task GetGameCenter(InvocationContext context)
		{
			...
			var gc = await StdLog(log?.SubPath(nameof(GetGameCenter)), async log =>
			{
				...
				var g = StdLog(log?.SubPath("Achievements"), async log =>
				{
					if (group.data != null)
					{
						// find achievements:
				...
				var d = ForEachAsync(gameCenterAchievements, async ach =>
				{
				...
				var e = ForEachAsync(gameCenterLeaderboards, async lb =>
				{
				...
				var f = ForEachAsync(gameCenterLeaderboardSets, async lbs =>
				{
		...
		private static async Task SetGameCenter(InvocationContext context)
		{
			...
			try
			{
				...
				await Task.WhenAll(
					ForEachAsync(gc.achievements, async ach =>
					{
						// TODO: make this a cli switch:
						if (ach.live == true)
							return;
						...
						{
							await ForEachAsync(ach.localizations, async loc =>
							{
					...
					ForEachAsync(gc.leaderboards, async lb =>
					{
						// TODO: make this a cli switch:
						if (lb.live == true)
							return;
						...
						{
							await ForEachAsync(lb.localizations, async loc =>
							{
								if (string.IsNullOrEmpty(loc.id))
								{
					...
					ForEachAsync(gc.leaderboardSets, async lbs =>
					{
						// TODO: make this a cli switch:
						if (lbs.live == true)
							return;
						...
						{
							await ForEachAsync(lbs.localizations, async loc =>
							{
								if (string.IsNullOrEmpty(loc.id))
								{
				...
				await ForEachAsync(gc.leaderboardSets, async lbs =>
				{
					// TODO: make this a cli switch:
					if (lbs.live == true)
						return;
				...
				await ForEachAsync(postLbsetActions.ToArray(), a => a());
		...
		private static void StdLog(INestedLog? log, Action<INestedLog?> body)
		{
			try
			{
		...
		private static T StdLog<T>(INestedLog? log, Func<INestedLog?, T> body)
		{
			try
			{
				log?.SetState(NestedLogState.Processing);
		...
		private static async Task StdLog(INestedLog? log, Func<INestedLog?, Task> body)
		{
			try
			{
				log?.SetState(NestedLogState.Processing);
		...
		private static async Task<T> StdLog<T>(INestedLog? log, Func<INestedLog?, Task<T>> body)
		{
			try
			{
				log?.SetState(NestedLogState.Processing);
		...
		private static Task ForEachAsync<T>(IEnumerable<T> source, Func<T, Task> body)
		{
			return Task.WhenAll(source.Select(body));
		}

		private static Task ForEachAsyncLog<T>(IEnumerable<T> source, INestedLog? log, Func<T, string> subLogName, Func<T, INestedLog?, Task> body)
		{
		...
		private sealed class ConsoleLogger(TextWriter m_TextWriter, int m_Verbosity)
			: INestedLog
		{
