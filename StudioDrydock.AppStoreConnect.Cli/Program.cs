...
namespace StudioDrydock.AppStoreConnect.Cli
{
    internal static class Program
    {
        ...
        private static AppStoreClient CreateClient(InvocationContext context, out INestedLog log)
        {
            ...
            {
                log = new ConsoleLogger(Console.Out, 3);
            }
            else
            {
                log = new ConsoleLogger(Console.Out, 0);
            }
        ...
        private static async Task GetAppVersions(InvocationContext context)
        {
            ...
            log = log?.SubPath(nameof(GetAppVersions));
            try
            {
                ...
                await ForEachAsync(versions, async version =>
                {
                    var vLog = log?.SubPath($"{version.platform}/{version.versionString}");
                    try
                    {
                        vLog?.SetState(NestedLogState.Processing);
                        ...
                        await ForEachAsync(localizations, async loc =>
                        {
                            var locLog = vLog?.SubPath($"{loc.locale}");
                            try
                            {
                                locLog?.SetState(NestedLogState.Processing);
                                ...
                                locLog?.SetState(NestedLogState.Success);
                            }
                            catch
                            {
                                locLog?.SetState(NestedLogState.Failure);
                                throw;
                            }
                        });
                        ...
                        vLog?.SetState(NestedLogState.Success);
                    }
                    catch
                    {
                        vLog?.SetState(NestedLogState.Failure);
                        throw;
                    }
                });

                Output(context, new App(appInfo, [.. versions]));
        ...
        private static async Task GetAppIaps(InvocationContext context)
        {
            ...
            await ForEachAsync(iaps, async iap =>
            {
            ...
            Output(context, new IapList([.. iaps.OrderBy(a => a.productId)]));
        }
        ...
        private static async Task GetAppEvents(InvocationContext context)
        {
            ...
            Output(context, new EventList([.. events.OrderBy(a => a.id)]));
        }
        ...
        private static async Task GetGameCenter(InvocationContext context)
        {
            ...
            await Task.WhenAll(
                ForEachAsync(gameCenterAchievements, async ach =>
                {
                ...
                ForEachAsync(gameCenterLeaderboards, async lb =>
                {
                ...
                ForEachAsync(gameCenterLeaderboardSets, async lbs =>
                {
            ...
            Output(context, new GameCenter(gameCenterDetail, gameCenterGroup, [.. gameCenterAchievements], [.. gameCenterLeaderboards], [.. gameCenterLeaderboardSets]));
        }
